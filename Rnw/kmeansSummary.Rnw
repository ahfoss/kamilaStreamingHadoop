...
dataName = '../csv/2001_rmvna_norm.csv'
JOBID = '5343534'

# A subsampled version of the data set suitable for plotting, ~1000 observations
subsampleData = '../csv/subsampled_2000_2001_rmvna_norm.csv'
@
...
for (varNum in 1:ncol(minMat)) {
  if (varNum <= numConVar) {
    ...
    ithProportion <- meanMat[,varNum] / maxMat[,varNum]
    print(
      xtable(
        data.frame(
	  ClusterNumber = 1:nrow(minMat),
	  Count = ithProportion * nLines,
	  Percent = ithProportion * 100
	),
	digits = c(0,0,0,1),
        caption = paste('Cluster-specific stats for variable ',varNum,', ',allVarNames[varNum],'.',sep='')
      ),
