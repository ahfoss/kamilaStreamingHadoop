...
dataName = '../csv/1987_rmvna_norm.csv'
JOBID = '5240978'

# A subsampled version of the data set suitable for plotting, ~1000 observations
subsampleData = '../csv/subsampled_1500_1987_rmvna_norm.csv'
@
...
print(
  xtable(
    varInfo,
    #digits = c(0,0,0,-2,-2),
    caption=c("Basic Dataset Info","Note: Mean and standard deviation reported before standardizing to mean 0 and sd 1")
  ),
...
clustInfoTab <- allStats[,c('ClusterNumber','ClusterSize','SSQ','wssPerN')]
colnames(clustInfoTab) <- c('Cluster Number','Cluster Size','Within-Cluster SS','WSS per point')
@
...
#varInfo
loads <- data.frame(
  Variable.Index = varInfo$Variable.Index,
  Variable.Name = varInfo$Variable.Name,
  PC1 = svdDat$v[,1],
  PC2 = svdDat$v[,2]
)
#loads
...
print(
  xtable(
    ...
    digits = c(0,0,3,3,3),
    caption = paste('Cluster-specific stats for variable ',varNum,', ',varInfo$Variable.Name[varNum],'.',sep='')
  ),
  include.rownames = FALSE
)
}
