...
for (i in 1:length(runDirs)) {
  ...
  nIter_i <- length(iterDirs_i)
  sortedIterDirs <- iterDirs_i[order(nchar(iterDirs_i), iterDirs_i)]
  objFileName <- list.files(
    path = iterDirs_i[[nIter_i]],
...
clustInfoTab <- read.table(file.path('..',outputFileName,paste('run_',best_run,sep=''),'stats','stats.tsv'))
...
\subsection{Variable-Specific Clustering Stats}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{../summary/kmeansPlot_5203020}
\end{center}
