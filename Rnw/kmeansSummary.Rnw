...
dataName = '../csv/cat1987-2008_v1_rmvna_norm.csv'
JOBID = '5344386'

# A subsampled version of the data set suitable for plotting, ~1000 observations
subsampleData = '../csv/subsampled_2000_cat1987-2008_v1_rmvna_norm.csv'
@
...
conVarInfo <- read.csv(paste(spl[1],'constats.',dataExt,sep=''))
...
for (varNum in 1:ncol(minMat)) {
  if (varNum <= numConVar) {
    ...
    ithClustCount <- ifelse(
      test = maxMat[,varNum] == 0,
      yes = 0,
      no = meanMat[,varNum] / maxMat[,varNum] * clustTotalCounts
    )
    print(
      xtable(
        data.frame(
	  ClusterNumber = 1:nrow(minMat),
          ...
          mean = meanMat[,varNum],
          max = maxMat[,varNum],
	  CountYes = ithClustCount,
	  Percent = ithClustCount / clustTotalCounts * 100
	),
