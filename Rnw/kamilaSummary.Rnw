...
dataName <- '../csv/sample_KAM_rmvna_norm.csv'
dataBaseName <- '../db/sample.db'
dataBaseTableName <- 'finalDataSet'
JOBID = '5761972'
# A subsampled version of the data set suitable for plotting
subsampleDataFile = '../csv/subsampled_2000_sample_KAM_rmvna_norm.csv'
...
<<fs,echo=FALSE>>=
fileSize <- file.info(dataName)$size
...
for (i in 1:length(catVarStats)) {
  print(
    xtable(
      ...
      caption = paste('Cluster-specific row percents for categorical variable ',i,', ',names(catVarStats)[i],'.',sep='')
    ),
